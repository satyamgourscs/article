    ...
    <div class="container py-4">
        ...
        @if ($current && $current->plan)
            <p class="text-muted">@lang('Current plan'): <strong>{{ __($current->plan->name) }}</strong>
                — @lang('Applies used'): {{ $current->jobs_applied_count }} /
                @if ($current->plan->job_apply_limit >= 999999)
                    @lang('Unlimited')
                @else
                    {{ $current->plan->job_apply_limit }}
                @endif
            </p>
        @endif
        <div class="row g-3">
                    ...
                    <div class="card h-100 border">
                        <div class="card-body d-flex flex-column">
                            ...
                            <ul class="small flex-grow-1">
                                <li>@lang('Applies'): {{ $plan->job_apply_limit >= 999999 ? __('Unlimited') : $plan->job_apply_limit }}</li>
                                <li>@lang('Job views'): {{ $plan->job_view_limit == 0 || $plan->job_view_limit >= 999999 ? __('Unlimited') : $plan->job_view_limit }}</li>
                                <li>@lang('Listing preview rows'): {{ $plan->listing_visible_jobs >= 999999 ? __('All') : $plan->listing_visible_jobs }}</li>
                            </ul>
                            @if ($current && $current->plan_id == $plan->id)
                                <button class="btn btn--dark btn--sm w-100" disabled>@lang('Current')</button>
                            @else
                                <form action="{{ route('user.subscription.pay', $plan->id) }}" method="post" class="mt-2">
                                    @csrf
