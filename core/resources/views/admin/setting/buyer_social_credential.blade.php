                ...
                <div class="card-body p-0">
                    <div class="table-responsive--sm table-responsive">
                        <table class="table table--light style--two custom-data-table">
                            ...
                            <tbody>
                                @foreach (gs('socialite_buyer_credentials') as $key => $credential)
                                    <tr>
                                        ...
                                        <td>
                                            <div class="button--group">
                                                ...
                                                @if (@$credential->status == Status::ENABLE)
                                                    <button class="btn btn-outline--danger btn-sm confirmationBtn"  data-question="@lang('Are you sure that you want to disable this login credential?')" data-action="{{ route('admin.setting.socialite.buyer.credentials.status.update', $key) }}">
                                                        <i class="las la-eye-slash"></i>@lang('Disable')
                                                    </button>
                                                @else
                                                    <button class="btn btn-outline--success btn-sm confirmationBtn" data-question="@lang('Are you sure that you want to enable login credential?')" data-action="{{ route('admin.setting.socialite.buyer.credentials.status.update', $key) }}">
    ...
    <script>
        (function($) {
            ...
            $(document).on('click', '.editBtn', function() {
                let modal = $('#editModal');
                let data = $(this).data();
                let route = "{{ route('admin.setting.socialite.buyer.credentials.update', '') }}";
                let callbackUrl = "{{ route('buyer.social.login.callback', '') }}";
