        ...
        <div class="sidebar__menu-wrapper">
            <ul class="sidebar__menu">
                @foreach($sideBarLinks as $key => $data)
                    @if(in_array($key, ['manage_jobs', 'manage_bids', 'manage_projects'], true) && !legacyBiddingEnabled())
                        @continue
                    @endif
                    @if (@$data->header)
                                        ...
                                        <li class="sidebar-menu-item {{ menuActive(@$menu->menu_active) }} ">
                                            <a href="{{ route(@$menu->route_name,$submenuParams) }}" class="nav-link">
                                                ...
                                                @php 
                                                    ...
                                                    $counterValue = isset($counter) ? (${$counter} ?? 0) : 0;
                                                @endphp
                                                @if($counterValue > 0)
                                                    <span class="menu-badge bg--info ms-auto">{{ $counterValue }}</span>
                                                @endif
                                            </a>
                                        </li>
                                    @endforeach
