            ...
            <tbody>
              @forelse($reports ?? [] as $report)
                <tr>
                    ...
                    <td><span class="badge badge--{{ @$report->status_class ?? 'secondary' }}">{{ @$report->status_text ?? __('N/A') }}</span></td>
                </tr>
              @empty
                <tr>
                    <td colspan="100%" class="text-center msg-center">{{ __($emptyMessage ?? 'No reports found') }}</td>
                </tr>
              @endforelse
            </tbody>
          </table><!-- table end -->
...
@push('breadcrumb-plugins')
    ...
    <a href="https://nexatechnologies.com/support" target="__blank" class="btn btn-sm btn-outline--success"><i class="las la-headset"></i> @lang('Request for Support')</a>
@endpush
