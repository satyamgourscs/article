        ...
        <div class="col-12">
            <div class="row gy-4">
                ...
                <div class="col-xxl-3 col-sm-6">
                    <x-widget style="7" link="{{ route('admin.report.transaction') }}?buyer_id={{ $buyer->id }}"
                        title="Balance" icon="las la-money-bill-wave-alt" value="{{ showAmount($buyer->balance) }}"
                        bg="indigo" type="2" />
                </div>


                <div class="col-xxl-3 col-sm-6">
                    <x-widget style="7" link="{{ route('admin.deposit.list', $buyer->id) }}" title="Deposits"
                        icon="las la-wallet" value="{{ showAmount($totalDeposit) }}" bg="8" type="2" />
                </div>

                <div class="col-xxl-3 col-sm-6">
                    <x-widget style="7"
                        link="{{ route('admin.withdraw.data.all') . '?buyer_id=' . $buyer->id }}" title="Withdrawals"
            ...
            <div class="d-flex flex-wrap gap-3 mt-4">
                ...
                <div class="flex-fill">
                    <a href="{{ route('admin.report.login.history') }}?search={{ $buyer->username }}"
                ...
                <div class="flex-fill">
                    <a href="{{ route('admin.buyers.notification.log', $buyer->id) }}"
                ...
                @if ($buyer->kyc_data)
                    <div class="flex-fill">
                        <a href="{{ route('admin.buyers.kyc.details', $buyer->id) }}" target="__blank"
            ...
            <div class="card mt-30">
                <div class="card-header">
                    <h5 class="card-title mb-0">@lang('Information of') {{ $buyer->fullname }}</h5>
                </div>
                <div class="card-body">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <div class="input-group ">
                                        <span class="input-group-text mobile-code">+{{ $buyer->dial_code }}</span>
                                        <input type="number" name="mobile" value="{{ $buyer->mobile }}"
                                    ...
                                    <select name="country" class="form-control select2">
                                        @foreach ($countries as $key => $country)
                                            <option data-mobile_code="{{ $country->dial_code }}"
                                                value="{{ $key }}" @selected($buyer->country_code == $key)>
                            ...
                            <div class="col-xl-3 col-12">
                                <div class="form-group">
                                    ...
                                    <input type="checkbox" data-width="100%" data-height="50" data-onstyle="-success"
                                        ...
                                        @if ($buyer->kv == Status::KYC_VERIFIED) checked @endif>
                                </div>
                            </div>
                            <div class="col-md-12">
    ...
    <div id="addSubModal" class="modal fade" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('admin.buyers.add.sub.balance', $buyer->id) }}"
    ...
    <div id="userStatusModal" class="modal fade" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('admin.buyers.status', $buyer->id) }}" method="POST">
                    @csrf
                    <div class="modal-body">
                        @if ($buyer->status == Status::USER_ACTIVE)
                            <h6 class="mb-2">@lang('If you ban this buyer he/she won\'t able to access his/her dashboard.')</h6>
                            <div class="form-group">
                        ...
                        @else
                            ...
                            <p>{{ $buyer->ban_reason }}</p>
                            <h4 class="text-center mt-3">@lang('Are you sure to unban this buyer?')</h4>
                        @endif
                    </div>
                    <div class="modal-footer">
                        @if ($buyer->status == Status::USER_ACTIVE)
...
@push('breadcrumb-plugins')
    <a href="{{ route('admin.buyers.login', $buyer->id) }}" target="__blank"
        class="btn btn-sm btn-outline--primary"><i class="las la-sign-in-alt"></i>@lang('Login as Firm')</a>
@endpush
