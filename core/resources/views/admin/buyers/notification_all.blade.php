            ...
            <div class="row  mb-4 justify-content-center">
                <div class="col-sm-7">
                    <div class="row gy-4 justify-content-center">
                        <div class="col-sm-6">
                            <x-widget link="javascript:void(0)" style="6" icon="fas fa-list" :title="$viaText . ' should be sent'" value="{{ @$sessionData['total_buyer'] }}"
                        ...
                        <div class="col-sm-6">
                            <x-widget link="javascript:void(0)" style="6" icon="fa-solid fa-paper-plane" :title="$viaText . ' has yet to be sent'" :viewMoreIcon=false
                                value="{{ @$sessionData['total_buyer'] - @$sessionData['total_sent'] }}" bg="warning" />
                        </div>
                        <div class="col-sm-6">
                                ...
                                <div class="card-body p-5 text-center">
                                    <div class="coaling-loader flex-column d-flex justify-content-center">
                                        ...
                                        <p class="text--primary">
                                            @lang(' ' . @$sessionData['total_sent'] . ' out of ' . @$sessionData['total_buyer'] . ' ' . $viaName . ' were successfully transmitted')
                                        </p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <select class="form-control select2" name="being_sent_to" required data-minimum-results-for-search="1">
                                        @foreach ($notifyToBuyer as $key => $toBuyer)
                                            <option value="{{ $key }}" @selected(old('being_sent_to', @$sessionData['being_sent_to']) == $key)>{{ __($toBuyer) }}</option>
                                        @endforeach
                                    </select>
                                    <small class="text--info d-none userCountText"> <i class="las la-info-circle"></i> <strong
                                            class="userCount">0</strong> @lang('active buyers found to send the notification')</small>
                                </div>
                                    ...
                                    <div class="col-md-4 start-from-col">
                                        <div class="form-group">
                                            ...
                                            <input class="form-control" name="start" value="{{ old('start', @$sessionData['start']) }}"
                                                type="number" placeholder="@lang('Start form buyer id. e.g. 1')" required>
                                        </div>
                                    </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="input-group">
                                                <input class="form-control" name="batch" value="{{ old('batch', @$sessionData['batch']) }}"
                                                    type="number" placeholder="@lang('How many buyer')" required>
    ...
    <script>
        ...
        (function($) {
            ...
            $('select[name=being_sent_to]').on('change', function(e) {
                ...
                getBuyerCount(methodName);
                methodName = methodName.toUpperCase();

                if (methodName == 'SELECTEDBUYERS') {
                    $('.input-append').html(`
                    <div class="form-group" id="user_list_wrapper">
                        <label class="required">@lang('Select Firm')</label>
                        <select name="buyer[]"  class="form-control" id="user_list" required multiple >
                ...
                if (methodName == 'TOPDEPOSITEDBUYERS') {
                    $('.input-append').html(`
                    <div class="form-group">
                        <label class="required">@lang('Number Of Top Deposited Firm')</label>
            ...
            function getBuyerCount(methodName) {
                ...
                var route = "{{ route('admin.buyers.segment.count', ':methodName') }}"
