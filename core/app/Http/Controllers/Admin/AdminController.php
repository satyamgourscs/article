...
class AdminController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        // Buyer Info
        $widget['total_buyers']             = Buyer::count();
        $widget['verified_buyers']          = Buyer::active()->count();
        $widget['email_unverified_buyers']  = Buyer::emailUnverified()->count();
        $widget['mobile_unverified_buyers'] = Buyer::mobileUnverified()->count();
        ...
        $widget['total_completed_projects'] = Project::completed()->count();
    ...
    public function requestReport()
    {
        ...
        // Updated support URL to Nexa Technologies
        $url = "https://nexatechnologies.com/support/api/issue/get?".http_build_query($arr);
        $response = CurlRequest::curlContent($url);
        $response = json_decode($response);
        if (!$response || !@$response->status || !@$response->message) {
            // Return empty reports array if API fails instead of redirecting
            $reports = [];
    ...
    public function reportSubmit(Request $request)
    {
        ...
        // Updated support URL to Nexa Technologies
        $url = 'https://nexatechnologies.com/support/api/issue/add';
        ...
        $response = json_decode($response);
        if (!$response || !@$response->status || !@$response->message) {
            $notify[] = ['error', 'Unable to submit report. Please contact support directly.'];
            return back()->withNotify($notify);
        }
        if ($response->status == 'error') {
