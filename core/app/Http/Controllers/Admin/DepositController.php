...
class DepositController extends Controller
{
    ...
    {
        if ($scope) {
            $deposits = Deposit::$scope()->with(['buyer', 'gateway']);
        }else{
            $deposits = Deposit::with(['buyer', 'gateway']);
        }
        ...
        $deposits = $deposits->searchable(['trx','buyer:username'])->dateFilter();
    ...
    {
        $deposit = Deposit::where('id', $id)->with(['buyer', 'gateway'])->firstOrFail();
        $pageTitle = $deposit->buyer->username.' requested ' . showAmount($deposit->amount);
