...
class GeneralSettingController extends Controller
{
    ...
    public function systemConfigurationSubmit(Request $request)
    {
        ...
        $general->buyer_registration = $request->buyer_registration ? Status::ENABLE : Status::DISABLE;
        $general->agree = $request->agree ? Status::ENABLE : Status::DISABLE;
        $general->multi_language = $request->multi_language ? Status::ENABLE : Status::DISABLE;
        $general->escrow_payment = $request->escrow_payment ? Status::ENABLE : Status::DISABLE;
        $general->job_auto_approved = $request->job_auto_approved ? Status::ENABLE : Status::DISABLE;
    ...
    public function socialiteBuyerCredentials()
    {
        $pageTitle = 'Buyer Social Login Credentials';
        return view('admin.setting.buyer_social_credential', compact('pageTitle'));
    }

    public function updateSocialiteBuyerCredentialStatus($key)
    {
        $general = gs();
        $credentials = $general->socialite_buyer_credentials;
        try {
            $credentials->$key->status = $credentials->$key->status == Status::ENABLE ? Status::DISABLE : Status::ENABLE;
        } catch (\Throwable $th) {
            abort(404);
        ...
        $general->socialite_buyer_credentials = $credentials;
        $general->save();

        $notify[] = ['success', 'Status changed successfully'];
        return back()->withNotify($notify);
    }
    public function updateSocialiteBuyerCredential(Request $request, $key)
    {
        $general = gs();
        $credentials = $general->socialite_buyer_credentials;
